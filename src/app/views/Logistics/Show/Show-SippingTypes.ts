...
@Component({
  ...
  templateUrl: './Show-SippingTypes.html',
...
export class ShippingTypesComponent implements OnInit {
  ...
  selectedType?: GlobalShippingTypesModel;
  ///// for toastr ////////
  position = 'top-end';
  ...
  confirmDelete(type: GlobalShippingTypesModel) {
    this.selectedType = type;
    this.showDeleteModal = true;
  }

  deleteFinantial(type?: GlobalShippingTypesModel) {
    if (!type) return;
    this.http.deleteData(`FinantialItem/${type.shippingTypeId}`,).subscribe(() => {
      this.GlobalShippingTypes = this.GlobalShippingTypes.filter(t => t.shippingTypeId !== type.shippingTypeId);
