...
@Component({
  ...
  templateUrl: './Show-Causes-types.html',
...
export class ShowCausesComponent implements OnInit {
  ...
  getCauses() {
    this.isLoading = true;
    this.http.getAllData('Cause').subscribe(
      (res: any) => {
        this.Causes = res
  ...
  deleteBusinessType(type?: any) {
    if (!type) return;
    this.http.deleteData(`Cause/${type.causeId}`,).subscribe((res) => {
      console.log(res)
      this.Causes = this.Causes.filter(t => t.causeId !== type.causeId);
      this.showDeleteModal = false;
      this.toastMessage.set(`${type.causeName} deleted successfully`);
      this.toastVisible.set(true);
    },(error) =>{
      this.toastMessage.set(`An error occured during delete (${type.causeName})`);
      this.toastVisible.set(true);
    });
  }
