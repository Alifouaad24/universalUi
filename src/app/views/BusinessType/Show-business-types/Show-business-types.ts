...
@Component({
  ...
  templateUrl: './Show-business-types.html',
...
export class ShowBusinessTypesComponent implements OnInit {
  ...
  getBusinessTypes() {
    this.isLoading = true;
    this.http.getAllData('BusinessType').subscribe(
      (res: any) => {
        this.businessTypes = (res as any[]).map(item => new BusinessType({
  ...
  confirmDelete(type: BusinessType) {
